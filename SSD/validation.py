...
def main():
    ...
    val_dataset = VOCDataset(VOC_root, transforms=data_transform["val"], file_name="val.txt")
    ...
    # 将验证结果保存至txt文件中
    with open(os.path.join(VOC_root, f"record_mAP_{cfg["weights"].split('.')[0]}.txt"), "w") as f:
