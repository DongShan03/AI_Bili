import torch

def generate_anchors(feats, fpn_strides, \
                    grid_cell_size=5.0, grid_cell_offset=0.5, \
                    device='cpu', is_eval=False, mode='af'):
    anchors = []
    anchor_points = []
    stride_tensor = []
    num_anchors_list = []

    assert feats is not None
    if is_eval:
        for i, stride in enumerate(fpn_strides):
            _, _, h, w = feats[i].shape
            #* 找到中心点
            #* shift_x -> [0, 1, ..., w-1] -> [w,]
            shift_x = torch.arange(end=w, device=device) + grid_cell_offset
            shift_y = torch.arange(end=h, device=device) + grid_cell_offset
            #* shift_x -> [h, w]   shift_y -> [h, w]
            shift_y, shift_x = torch.meshgrid(shift_y, shift_x, indexing='ij')
            #* anchor_point -> [h, w, 2]
            anchor_point = torch.stack([shift_x, shift_y], -1).to(torch.float)
            if mode == 'af':    #* anchor free
                #* anchor_points -> [-1, h * w, 2]
                anchor_points.append(anchor_point.reshape([-1, 2]))
                #* stride_tensor -> [-1, h * w, 1]
                stride_tensor.append(
                    torch.full(
                        (h * w, 1), stride, dtype=torch.float, device=device
                    )
                )
            elif mode == 'ab':  #* anchor base
                #* anchor_points -> [-1, h * w * 3, 2]
                anchor_points.append(anchor_point.reshape([-1, 2]).repeat(3, 1))
                #* stride_tensor -> [-1, h * w * 3, 1]
                stride_tensor.append(
                    torch.full(
                        (h * w, 1), stride, dtype=torch.float, device=device
                    ).repeat(3, 1)
                )

        anchor_points = torch.cat(anchor_points, 0)
        stride_tensor = torch.cat(stride_tensor, 0)
        return anchor_points, stride_tensor

    else:
        for i, stride in enumerate(fpn_strides):
            _, _, h, w = feats[i].shape
            cell_half_size = grid_cell_size * stride * 0.5
            shift_x = (torch.arange(end=w, device=device) + grid_cell_offset) * stride
            shift_y = (torch.arange(end=h, device=device) + grid_cell_offset) * stride
            shift_y, shift_x = torch.meshgrid(shift_y, shift_x, indexing='ij')
            anchor = torch.stack(
                [
                    shift_x - cell_half_size,
                    shift_y - cell_half_size,
                    shift_x + cell_half_size,
                    shift_y + cell_half_size,
                ],
                dim=-1
            ).clone().to(feats[0].dtype)

            anchor_point = torch.stack(
                [shift_x, shift_y], dim=-1
            ).clone().to(feats[0].dtype)

            if mode == 'af':
                anchors.append(anchor.reshape([-1, 4]))
                anchor_points.append(anchor_point.reshape([-1, 2]))
            elif mode == 'ab':
                anchors.append(anchor.reshape([-1, 4]).repeat(3, 1))
                anchor_points.append(anchor_point.reshape([-1, 2]).repeat(3, 1))

            num_anchors_list.append(len(anchors[-1]))
            stride_tensor.append(
                torch.full(
                    [num_anchors_list[-1], 1], stride, dtype=feats[0].dtype
                )
            )
        anchors = torch.cat(anchors, 0)
        anchor_points = torch.cat(anchor_points, 0).to(device)
        stride_tensor = torch.cat(stride_tensor, 0).to(device)
        return anchors, anchor_points, num_anchors_list, stride_tensor
