...
class VOCDataset(Dataset):
    #! voc_root -> VOC2012
    def __init__(self, voc_root, transforms=None, file_name="train.txt"):
        self.img_root = os.path.join(voc_root, "JPEGImages")
        self.annotations_root = os.path.join(voc_root, "Annotations")
        txt_list = os.path.join(voc_root, file_name)
        with open(txt_list, 'r') as read:
            self.xml_list = [os.path.join(self.annotations_root, line.strip() + ".xml")
                            for line in read.readlines() if len(line.strip()) > 0]
        json_file = os.path.join(voc_root, "pascal_voc_classes.json")
        if not os.path.exists(json_file):
            print("Creating a class index file")
            names = []
            for xml in tqdm(self.xml_list, desc="reading xml"):
                with open(xml, "r") as f:
                    objs = self.parse_xml_to_dict(etree.fromstring(f.read()))["annotation"]["object"]
                    for obj in objs:
                        name = obj["name"]
                        if name not in names:
                            names.append(name)
                    f.close()
            class_dict = dict(zip(sorted(names), range(1, len(names)+1)))
            if file_name == "train.txt":
                json.dump(class_dict, open(json_file, "w"), indent=4)
    ...
    def __getitem__(self, idx):
        ...
        assert "object" in data, "xml contains no object"
        boxes = []
        labels = []
        iscrowd = []
...
if __name__ == "__main__":
    VOCDataset(r"D:\\pythonProject\\AI_from_bili\\data\\VOC2012")
