...
from YOLOv8.utils.coco_eval import CocoEvaluator
from YOLOv8.utils.coco_utils import get_coco_api_from_dataset
import YOLOv8.utils.distributed_utils as utils
...
def train_one_epoch(model, optimizer, data_loader,
    ...
    for i, (label, _) in pbar:
        ...
        with torch.amp.autocast("cuda", enabled=scaler is not None):
            ...
            loss, loss_items = computeLoss(pred, label)  # loss scaled by batch_size
        ...
        s = ('Epochs: %g/%g ' + 'MEM: %4s ' + 'lr:%.6f ' + 'lbox:%8.4g ' + 'lcls:%8.4g ' + 'ldfl:%8.4g ' + "loss:%8.4g") % \
...
def evaluate(model, data_loader, coco=None, device=None):
    ...
    for label, img_index in tqdm(data_loader, desc="Evaluating for coco evaluator: "):
        imgs = label["img"]
        shapes = label["ori_shape"][::1]
        imgs = imgs.to(device).float() / 255.0  # uint8 to float32, 0 - 255 to 0.0 - 1.0
        ...
        for index, p in enumerate(pred):
            if p is None:
                ...
                boxes = scale_coords(imgs[index].shape[1:], boxes, shapes[index]).round()
