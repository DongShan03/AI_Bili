...
class Bboxes:
    def __init__(self, bboxes, format="xyxy"):
        assert format in _formats, f"format must be one of {_formats}"
    ...
    def areas(self):
        return (
            ...
            if self.format == "xyxy"
            else self.bboxes[:, 2] * self.bboxes[:, 3]
        )
    ...
    @staticmethod
    def concatenate(cls, boxes_list, axis=0):
...
class Instances:
    ...
    def __getitem__(self, index):
        ...
        keypoints = self.keypoints[index] if self.keypoints is not None and len(self.segments) else self.keypoints
        bboxes = self._bboxes[index].bboxes
    ...
    def concatenate(cls, instances_list, axis=0):
        ...
        if not instances_list:
            return cls(np.empty(0))
        assert all(isinstance(inst, Instances) for inst in instances_list)

        if len(instances_list) == 1:
            return instances_list[0]

        use_segments = instances_list[0].segments is not None
        use_keypoints = instances_list[0].keypoints is not None
        bbox_format = instances_list[0]._bboxes.format
        normalized = instances_list[0].normalized

        cat_boxes = np.concatenate([ins._bboxes for ins in instances_list], axis=axis)
        if use_segments:
        ...
        cat_keypoints = np.concatenate([b.keypoints for b in instances_list], axis=axis) if use_keypoints else None
        return cls(cat_boxes, cat_segments, cat_keypoints, format=bbox_format, normalized=normalized)
