...
from cfg import cfg
from utils import train_one_epoch, evaluate, preprocess, get_params_groups, create_lr_scheduler, plot_final
...
def train(net_name, epochs=20, learn_rate=0.0002):
    ...
    pg = get_params_groups(cfg['net'], weight_decay=cfg["weight_decay"])
    optimizer = optim.AdamW(pg, lr=learn_rate, weight_decay=cfg["weight_decay"])
    lr_scheduler = create_lr_scheduler(optimizer, len(train_loader), cfg["epochs"],
