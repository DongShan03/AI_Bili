...
from YOLOv7.model.layers import *
from YOLOv7.utils.loss import SigmoidBin
from YOLOv7.utils.utils import (make_divisible, check_anchor_order, initialize_weights, fuse_conv_and_bn, \
                                model_info, copy_attr, scale_img, time_synchronized)
try:
...
class IKeypoint(nn.Module):
    ...
    def __init__(self, nc=80, anchors=(), nkpt=17, ch=(), inplace=True, dw_conv_kpt=False):
        ...
        if self.nkpt is not None:
            if self.dw_conv_kpt:
                #* keypoint head is slightly more complex
                self.m_kpt = nn.ModuleList(
                    nn.Sequential(
                        ...
                        DWConv(x, x, k=3), Conv(x, x),
                        DWConv(x, x, k=3), Conv(x, x),
                        DWConv(x, x, k=3), nn.Conv2d(x, self.no_kpt * self.na, 1)
                    ) for x in ch
                )
            else:
                #* keypoint head is a single convolution
        ...
        for i in range(self.nl):
            ...
            if not self.training:
                ...
                if self.inplace:
                    ...
                    if self.nkpt != 0:
                        x_kpt[..., 0::3] = (x_kpt[..., 0::3] * 2. - 0.5 + kpt_grid_x.repeat(1, 1, 1, 1, 17)) * self.stride[i]
...
class IBin(nn.Module):
    def __init__(self, nc=80, anchors=(), ch=(), bin_count=21):
        ...
        self.grid = [torch.ones(1)] * self.nl
        a = torch.tensor(anchors).float().view(self.nl, -1, 2)
        self.register_buffer('anchors', a)
        self.register_buffer('anchor_grid', a.clone().view(self.nl, 1, -1, 1, 1, 2))
        self.m = nn.ModuleList(nn.Conv2d(x, self.no * self.na, 1) for x in ch)
...
class Model(nn.Module):
    def __init__(self, cfg="yolov7-tiny.yaml", ch=3, nc=None, anchors=None):
        super().__init__()
    ...
    def _initialize_bin_biases(self, cf=None):
        m = self.model[-1]
        bc = m.bin_count
        for mi, s in zip(m.m, m.stride):
            b = mi.bias.view(m.na, -1)
    ...
    def _initialize_kpt_biases(self, cf=None):
        m = self.model[-1]
        for mi, s in zip(m.m, m.stride):
            b = mi.bias.view(m.na, -1)
            b.data[:, 4] += math.log(8 / (640 / s) ** 2)
    ...
    def fuse(self):
        ...
        for m in self.model.modules():
            ...
            elif isinstance(m, (IAuxDetect, IDetect)):
                m.fuse()
                m.forward = m.fuseforward
        return self
